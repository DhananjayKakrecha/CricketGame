...
<html lang="en">
  ...
  <body>
    <%
	CricketPlayer player = (CricketPlayer) session.getAttribute("cpuPlayer");
	...
	if(role.equals("Bat First")){
		if(player.getTotalScore() > targetScore){
			session.setAttribute("reason","Opponent Achieved Target!");
	...
	%>
		<button type="button"><a href="cricketplay.jsp">BAT</a></button>
	<% 		
		}else{
			if(targetScore == player.getTotalScore()){
				...
				session.setAttribute("reason","OUT!!, Opponent didn't achive the target!");
	...
	<% 		
			...
			if(targetScore == player.getTotalScore()){
				...
				session.setAttribute("reason","OVers finished, Opponent didn't achived the target!");
				response.sendRedirect("Won.jsp");
			}
    ...
    <form id="timeOut" style="display: none;" action="Perform"
		method="post">
		<input type="hidden" name="action" value="ballTimeOut"> <input
			type="submit">
	</form>
